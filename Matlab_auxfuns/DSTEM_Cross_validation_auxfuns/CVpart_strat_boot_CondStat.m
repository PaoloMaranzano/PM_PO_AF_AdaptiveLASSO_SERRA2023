%% CV partinioning: stratified bootstrap cross-validation
function [cv_part] = CVpart_strat_boot_CondStat(DSTEM_str_ground,Strat_var,Runs,Perc)
...
for run = 1:Runs
    for p = 1:length(cv_part.poll)
        ...
        cv_part.mat_fold{1,run}{1,p} = DSTEM_str_ground.([cv_part.poll{p}]);
        ...
        for row = 1:size(cv_part.mat_fold{1,run}{1,p},1)
            % Non-missing values among the selected stations
            nonnan_y = ~isnan(cv_part.mat_fold{1,run}{1,p}(row,:));
            % Count the non-missing values
            lng = length(find(nonnan_y));
            % Assign to all non-missing values in r-th row a value in 1 to K
            v = [repmat(1, floor(lng*Perc),1)' , repmat(0, lng-floor(lng*Perc),1)'];
            % Random assignment to a folder (re-shuffle) by row
            cv_part.mat_fold{1,run}{1,p}(row,nonnan_y) = datasample(v,lng,'Replace',false);
        end
        % Identify which stations monitor the p-th pollutant
