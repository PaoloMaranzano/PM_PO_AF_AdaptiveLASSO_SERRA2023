%% %%%%%%%%%% DSTEM_KfoldCV_par
%%% Evaluate K-fold CV performances of a DSTEM model.
%%% See CVpart for avaiable CV schemes

function [CV_perf_metrics] = DSTEM_KfoldCV(cv_part,DSTEM_model,ActiveSets,ASi_idx)
...
for run = 1:cv_part.K
    ...
    obj_temp_stem_EM_options.exit_tol_loglike = DSTEM_model.stem_EM_result.exit_tol_loglike*5;
    obj_temp_stem_EM_options.exit_tol_par = DSTEM_model.stem_EM_result.exit_tol_par*5;
    ...
    for p = 1:nvarY
        for i = 1:size(cv_part.test_coords{1,run}{1,p},1)
            ...
            y_oos.(namevarY{p})(i,4) = obj_temp_stem_model.stem_EM_result.y_hat{1,p}(cv_part.test_coords{1,run}{1,p}(i,1),...
                cv_part.test_coords{1,run}{1,p}(i,2));
        end
